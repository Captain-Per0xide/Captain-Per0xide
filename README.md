<p align="center">
  <a href="https://linkedin.com/in/ranju-mallik" target="_blank">
    <img src="github-header-image.png" alt="Ranju Mallik – Software Developer Banner"/>
  </a>
</p>

<h1 align="center">Ranju Mallik</h1>
<h3 align="center">Software Developer | Freelance Full-Stack Engineer</h3>

---

## About Me
I am a **Freelance Full-Stack Developer** with experience building scalable web and mobile applications.  
I work across the complete development lifecycle—from requirements gathering to deployment—while ensuring performance, security, and maintainability.

- Currently working with **Next.js, React Native, and Supabase**
- Open to collaboration on **web and mobile application projects**
- Strong foundation in **cybersecurity and system reliability**
- Comfortable working independently or with cross-functional teams

---

## Connect With Me
[![LinkedIn](https://skillicons.dev/icons?i=linkedin)](https://linkedin.com/in/ranju-mallik)
&nbsp;
[![GitHub](https://skillicons.dev/icons?i=github)](https://github.com/Captain_Per0xide)
&nbsp;
[![Email](https://skillicons.dev/icons?i=gmail)](mailto:mallikranju294@gmail.com)

---

## Technical Skills
[![Tech Stack](https://skillicons.dev/icons?i=js,ts,react,nextjs,reactnative,nodejs,php,java,golang,rust,flutter,electron,supabase,mongodb,postgres,git,linux&perline=10)](https://skillicons.dev)

---

## Projects

### Educational App (Jun 2025 – Present)
- Developed using **React Native** with **Supabase** backend
- Implemented video uploads, payment integration, and push notifications
- Designed role-based access for multiple user types
- Built a coding league system with real-time data handling
- Focused on responsive UI and smooth user experience

### E-Commerce Website (Jan 2024 – Aug 2024)
- Full-stack application built with **Next.js and Supabase**
- Implemented secure authentication and admin dashboard
- Designed responsive UI for a computer retail business
- Managed product listings, orders, and user roles

---

## GitHub Statistics
![](https://github-readme-stats.vercel.app/api?username=Captain_Per0xide&show_icons=true&theme=radical)
![](https://github-readme-streak-stats.herokuapp.com/?user=Captain_Per0xide&theme=radical)
![](http://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Captain_Per0xide&theme=radical)

---

## GitHub Achievements
![](https://github-profile-trophy.vercel.app/?username=Captain_Per0xide&theme=radical&no-frame=false&no-bg=true&margin-w=5)

---

## Education
**Bachelor of Science in Computer Science**  
*Maharaja Manindra Chandra College, Kolkata*  
Aug 2023 – Present

---

## Languages
- Bengali — Native  
- Hindi — Advanced  
- English — Proficient  

---

⭐ *If you find my work useful, feel free to explore and star my repositories.*
